import { ai as useAsyncData, g as _sfc_main$2$1, h as _sfc_main$7, i as _sfc_main$8, aj as useAuth, $ as $fetch$2, aM as useToast, aJ as useRuntimeConfig } from '../virtual/entry.mjs';
import { _ as _sfc_main$3 } from './Select-QzRNfVS-.mjs';
import { _ as _sfc_main$a } from './FormField-DOShaxcI.mjs';
import { _ as _sfc_main$1 } from './Pagination-D8tenX5M.mjs';
...
import { b as _sfc_main$2, a as _sfc_main$1$1, _ as _sfc_main$5 } from './DashboardSidebarCollapse-CLHbm061.mjs';
import { _ as _sfc_main$9 } from './Form-DpngGyYw.mjs';
import { _ as _sfc_main$b } from './Textarea-CPf4yHZZ.mjs';
import { defineComponent, ref, withAsyncContext, computed, mergeProps, withCtx, unref, isRef, openBlock, createBlock, createVNode, Fragment, renderList, toDisplayString, createCommentVNode, reactive, watch, useSSRContext } from 'vue';
...
function useReconciliations() {
	...
	async function listCutoffs(page = 1) {
		return (await $fetch$2(`${config.public.apiBase}/cutoffs?page=${page}&per_page=50`, { headers: { Authorization: `Bearer ${token.value}` } })).data;
	}
	async function listCutoffRelations(cutoffId) {
		return (await $fetch$2(`${config.public.apiBase}/cutoffs/${cutoffId}`, { headers: { Authorization: `Bearer ${token.value}` } })).data.relations ?? [];
	}
					...
					if (_push) _push(ssrRenderComponent(_component_UDashboardNavbar, { title: "Conciliaciones" }, {
						...
						right: withCtx((_, _push, _parent, _scopeId) => {
							if (_push) {
								...
								placeholder: "Estado",
								class: "w-40"
							}, null, 8, ["modelValue", "onUpdate:modelValue"]), createVNode(_component_UInput, {
								modelValue: unref(reference),
								"onUpdate:modelValue": [($event) => isRef(reference) ? reference.value = $event : null, onSearch],
								placeholder: "Buscar por referencia...",
								icon: "i-lucide-search",
					...
					if (_push) if (unref(status) === "pending") {
						...
						if (unref(items).length === 0) {
							...
							ssrRenderList(unref(items), (item) => {
								...
								if (!item.reconciled) _push(ssrRenderComponent(_component_ReconciliationsManualMatchModal, {
									transaction: item,
									onMatched
								}, null, _parent, _scopeId));
								else _push(`<!---->`);
						...
						])])]), createVNode("div", { class: "flex items-center gap-3" }, [createVNode("p", { class: "text-sm font-semibold text-highlighted" }, toDisplayString(unref(money).format(Number(item.amount))), 1), !item.reconciled ? (openBlock(), createBlock(_component_ReconciliationsManualMatchModal, {
							key: 0,
							transaction: item,
							onMatched
...
//# sourceMappingURL=reconciliations-BG7jOHWF.mjs.map
