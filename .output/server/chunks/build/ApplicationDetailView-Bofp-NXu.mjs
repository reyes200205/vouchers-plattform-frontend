...
var ApplicationDetailView_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const coordinatorPhotos = computed(() => {
			const items = [];
			if (props.detail.id_front_url) items.push({
				label: "INE (frente)",
				url: props.detail.id_front_url
			});
			if (props.detail.id_back_url) items.push({
				label: "INE (reverso)",
				url: props.detail.id_back_url
			});
			if (props.detail.proof_of_address_url) items.push({
				label: "Comprobante de domicilio",
				url: props.detail.proof_of_address_url
			});
			return items;
		});
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (unref(coordinatorPhotos).length) {
				...
				ssrRenderList(unref(coordinatorPhotos), (photo) => {
					_push(`<a${ssrRenderAttr("href", photo.url)} target="_blank" rel="noopener" class="group block overflow-hidden rounded-lg border border-default"><img${ssrRenderAttr("src", photo.url)}${ssrRenderAttr("alt", photo.label)} class="aspect-square w-full object-cover transition group-hover:opacity-80"><p class="truncate px-2 py-1 text-xs text-muted">${ssrInterpolate(photo.label)}</p></a>`);
				});
				_push(`<!--]--></div></div>`);
			} else _push(`<!---->`);
...
//# sourceMappingURL=ApplicationDetailView-Bofp-NXu.mjs.map
