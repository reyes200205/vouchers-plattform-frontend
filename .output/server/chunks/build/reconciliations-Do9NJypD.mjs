...
import { _ as _sfc_main$9 } from './Tabs-FmegTH4u.mjs';
import { _ as _sfc_main$a } from './Modal-C3ktQuxc.mjs';
...
import { _ as _sfc_main$b } from './Form-CCmdJDgC.mjs';
import { u as useBranches } from './useBranches-DcaZOH57.mjs';
import { _ as _sfc_main$d } from './Textarea-DLoRbkWE.mjs';
import { u as useReconciliations } from './useReconciliations-D-H88Ltl.mjs';
import { D as DecideReconciliationModal_default, _ as _sfc_main$8 } from './DecideReconciliationModal-Nfej1igU.mjs';
					...
					if (_push) if (unref(selectedTab) === "movements") {
						...
						if (unref(pendingStatus) === "pending") {
							...
							ssrRenderList(unref(pendingItems), (item) => {
								...
								_push(`<div class="min-w-0"${_scopeId}><div class="flex items-center gap-2"${_scopeId}><p class="truncate font-semibold text-highlighted"${_scopeId}>${ssrInterpolate(item.distributor_payment?.distributor?.name || item.distributor_payment?.reported_reference || `Conciliación #${item.id}`)}</p>`);
								...
								if (item.distributor_payment?.cutoff_relation?.cutoff?.branch_name) _push(`<span${_scopeId}>${ssrInterpolate(item.distributor_payment.cutoff_relation.cutoff.branch_name)}</span>`);
								else _push(`<!---->`);
								if (item.notes) _push(`<span${_scopeId}>${ssrInterpolate(item.notes)}</span>`);
								else _push(`<!---->`);
								_push(`</div><p class="mt-1 text-xs text-dimmed"${_scopeId}> Relación ${ssrInterpolate(item.distributor_payment?.cutoff_relation?.relation_number ?? `#${item.distributor_payment?.cutoff_relation_id ?? "—"}`)} · Diferencia: ${ssrInterpolate(unref(money).format(Number(item.amount_difference)))}</p></div></div><div class="flex items-center gap-3"${_scopeId}><div class="text-right"${_scopeId}><p class="text-xs text-muted"${_scopeId}> Conciliado: </p><p class="text-sm font-semibold text-highlighted"${_scopeId}>${ssrInterpolate(unref(money).format(Number(item.reconciled_amount)))}</p></div>`);
						...
						return openBlock(), createBlock("div", {
							...
							createVNode("div", { class: "flex items-center gap-2" }, [createVNode("p", { class: "truncate font-semibold text-highlighted" }, toDisplayString(item.distributor_payment?.distributor?.name || item.distributor_payment?.reported_reference || `Conciliación #${item.id}`), 1), createVNode(_component_UBadge, {
								color: "warning",
								variant: "subtle",
								label: "Por verificar"
							})]),
							createVNode("div", { class: "flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-muted" }, [
								createVNode("span", null, " Registrada " + toDisplayString(item.reconciled_at ? new Date(item.reconciled_at).toLocaleDateString("es-MX") : ""), 1),
								item.distributor_payment?.cutoff_relation?.cutoff?.branch_name ? (openBlock(), createBlock("span", { key: 0 }, toDisplayString(item.distributor_payment.cutoff_relation.cutoff.branch_name), 1)) : createCommentVNode("", true),
								item.notes ? (openBlock(), createBlock("span", { key: 1 }, toDisplayString(item.notes), 1)) : createCommentVNode("", true)
							]),
							createVNode("p", { class: "mt-1 text-xs text-dimmed" }, " Relación " + toDisplayString(item.distributor_payment?.cutoff_relation?.relation_number ?? `#${item.distributor_payment?.cutoff_relation_id ?? "—"}`) + " · Diferencia: " + toDisplayString(unref(money).format(Number(item.amount_difference))), 1)
						])]), createVNode("div", { class: "flex items-center gap-3" }, [createVNode("div", { class: "text-right" }, [createVNode("p", { class: "text-xs text-muted" }, " Conciliado: "), createVNode("p", { class: "text-sm font-semibold text-highlighted" }, toDisplayString(unref(money).format(Number(item.reconciled_amount))), 1)]), createVNode(_component_ReconciliationsDecideReconciliationModal, {
							item,
							onDecided
						}, null, 8, ["item"])])])]);
					}), 128))]))], 64))];
...
//# sourceMappingURL=reconciliations-Do9NJypD.mjs.map
