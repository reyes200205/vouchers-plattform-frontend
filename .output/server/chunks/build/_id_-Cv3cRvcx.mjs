import { aK as useRoute, al as useAuth, aP as useToast, aL as useRouter, ak as useAsyncData, j as _sfc_main$2, f as _sfc_main$9 } from '../virtual/entry.mjs';
import { _ as _sfc_main$7 } from './Select-5EkiFePr.mjs';
import { _ as _sfc_main$5 } from './FormField-BitybEBm.mjs';
import { _ as _sfc_main$8 } from './Switch-BVVTXEME.mjs';
...
import { a as _sfc_main$1, _ as _sfc_main$a } from './DashboardNavbar-BUcs0a3E.mjs';
import { _ as _sfc_main$3 } from './Form-CCmdJDgC.mjs';
import { u as useBranches } from './useBranches-DcaZOH57.mjs';
import { _ as _sfc_main$4 } from './Card-CmMDF934.mjs';
import { u as useStaff } from './useStaff-C489DQX_.mjs';
import { _ as _sfc_main } from './Alert-CSACBiI_.mjs';
import { defineComponent, ref, computed, withAsyncContext, reactive, watch, mergeProps, withCtx, unref, createVNode, openBlock, createBlock, useSSRContext } from 'vue';
...
//#region app/pages/general/staff/[id].vue?vue&type=script&setup=true&lang.ts
var _id__vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	async setup(__props) {
		...
		const { data: branches } = ([__temp, __restore] = withAsyncContext(() => useAsyncData("staff-edit-branches", () => listBranches(), { default: () => [] })), __temp = await __temp, __restore(), __temp);
		const { data: member, status: memberStatus } = ([__temp, __restore] = withAsyncContext(() => useAsyncData("staff-edit-member-" + memberId, () => getStaff(memberId), "$z4gKCfiWhx")), __temp = await __temp, __restore(), __temp);
		const roles = ref([]);
		...
		const branchItems = computed(() => {
			...
			if (isBranchManager.value) available = available.filter((b) => b.id === branchManagerBranchId.value);
			else if (state.role_code === "general_manager") available = available.filter((b) => !b.manager || b.id === member.value?.home_branch?.id);
		...
		watch(member, (val) => {
			if (val) {
				...
				state.gender = val.person?.gender ?? void 0;
				...
				state.branch_id = state.role_code === "general_manager" ? val.home_branch ? String(val.home_branch.id) : void 0 : String(val.roles.find((r) => r.is_primary)?.branch_id ?? val.roles[0]?.branch_id ?? "");
				state.is_active = val.is_active;
			}
		}, { immediate: true });
		watch(() => state.role_code, () => {
			if (!branchItems.value.some((b) => b.value === state.branch_id)) state.branch_id = void 0;
		});
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(_component_UDashboardPanel, mergeProps({ id: "staff-edit" }, _attrs), {
				header: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(ssrRenderComponent(_component_UDashboardNavbar, { title: unref(member) ? `Editar: ${unref(member).person?.first_name} ${unref(member).person?.last_name}` : "Editar Miembro" }, {
					...
					else return [createVNode(_component_UDashboardNavbar, { title: unref(member) ? `Editar: ${unref(member).person?.first_name} ${unref(member).person?.last_name}` : "Editar Miembro" }, {
				...
				body: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) if (unref(memberStatus) === "pending") {
										...
										default: withCtx((_, _push, _parent, _scopeId) => {
											if (_push) {
												...
												_push(ssrRenderComponent(_component_UFormField, {
													...
													label: "Sucursal",
													description: unref(state).role_code === "general_manager" ? "Opcional: sucursal base. El gerente general conserva acceso a todas las sucursales." : void 0,
													name: "branch_id"
												}, {
													default: withCtx((_, _push, _parent, _scopeId) => {
														if (_push) _push(ssrRenderComponent(_component_USelect, {
					...
					else return [unref(memberStatus) === "pending" ? (openBlock(), createBlock("div", {
						key: 0,
						class: "flex items-center justify-center py-10"
					}, [createVNode(_component_UIcon, {
						name: "i-lucide-loader-2",
						class: "size-8 animate-spin text-muted"
					})])) : !unref(member) ? (openBlock(), createBlock("div", {
						key: 1,
						class: "p-6"
...
//#endregion
//#region app/pages/general/staff/[id].vue
...
_id__vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
	...
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/general/staff/[id].vue");
	return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
var _id__default = _id__vue_vue_type_script_setup_true_lang_default;

export { _id__default as default };
//# sourceMappingURL=_id_-Cv3cRvcx.mjs.map
