...
var new_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	async setup(__props) {
		...
		const schema = z.object({
			...
			password: z.string().min(22, "Mínimo 22 caracteres").regex(/[a-z]/, "Debe contener al menos una letra minúscula").regex(/[A-Z]/, "Debe contener al menos una letra mayúscula").regex(/[0-9]/, "Debe contener al menos un número").regex(/[^A-Za-z0-9]/, "Debe contener al menos un carácter especial o símbolo"),
		...
		const generateStrongPassword = () => {
			...
			password += special[Math.floor(Math.random() * 14)];
			const allChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+?~";
			for (let i = 0; i < 20; i++) password += allChars[Math.floor(Math.random() * 76)];
			password = password.split("").sort(() => .5 - Math.random()).join("");
			state.password = password;
			showPassword.value = true;
			if ((void 0).clipboard) {
				(void 0).clipboard.writeText(password);
				toast.add({
		...
		const branchItems = computed(() => {
			...
			else if (state.role_code === "general_manager") available = available.filter((b) => !b.manager);
		...
		async function onSubmit(event) {
			...
			try {
				await createStaff({
					...
					branch_id: event.data.branch_id ? Number(event.data.branch_id) : void 0
				});
				toast.add({
										...
										default: withCtx((_, _push, _parent, _scopeId) => {
											if (_push) {
												...
												_push(ssrRenderComponent(_component_UFormField, {
													...
													default: withCtx((_, _push, _parent, _scopeId) => {
														if (_push) {
															_push(`<div class="flex gap-2 items-center w-full"${_scopeId}>`);
															...
															_push(`</div>`);
														} else return [createVNode("div", { class: "flex gap-2 items-center w-full" }, [
												...
												createVNode(_component_UFormField, {
													...
													default: withCtx(() => [createVNode("div", { class: "flex gap-2 items-center w-full" }, [
														createVNode(_component_UInput, {
															modelValue: unref(state).password,
															"onUpdate:modelValue": ($event) => unref(state).password = $event,
															type: unref(showPassword) ? "text" : "password",
...
//# sourceMappingURL=new-GPIyobNq.mjs.map
