...
import { _ as _sfc_main$4 } from './DropdownMenu-BoWa7GET.mjs';
...
import { _ as _sfc_main$c } from './Input-BC1I0LeZ.mjs';
...
var RelationDetailModal_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const normalPortion = computed(() => {
			return (props.relation?.items ?? []).filter((item) => !isCarryover(item)).reduce((sum, item) => sum + remainingOf(item), 0);
		});
		const carryoverPortion = computed(() => {
			return (props.relation?.items ?? []).filter(isCarryover).reduce((sum, item) => sum + remainingOf(item), 0);
		});
			...
			_push(ssrRenderComponent(_component_UModal, mergeProps({
				...
				body: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) if (__props.relation) {
						...
						_push(`</div>`);
						if (__props.relation.retroactive_reconciliation) {
							_push(`<div class="rounded-lg border border-success/40 bg-success/10 p-4 text-sm"${_scopeId}><p class="mb-1 text-xs font-medium uppercase text-success"${_scopeId}> Pagada por conciliación manual </p><p class="text-muted"${_scopeId}> El depósito real sí llegó a tiempo según el banco: un gerente lo verificó el ${ssrInterpolate(fmtDateTime(__props.relation.retroactive_reconciliation.verified_at))}`);
							if (Number(__props.relation.retroactive_reconciliation.waived_late_fees_total ?? 0) > 0) _push(`<span${_scopeId}> y se le quitó ${ssrInterpolate(fmtMoney(__props.relation.retroactive_reconciliation.waived_late_fees_total))} de recargo por atraso que se había aplicado por error</span>`);
							else _push(`<!---->`);
							_push(`. Esta relación se quedó como CERRADA porque su deuda ya se arrastró a un corte más nuevo, pero ya no tiene ningún adeudo pendiente por esta causa. </p></div>`);
						} else _push(`<!---->`);
						if (unref(hasCarryover)) {
							_push(`<div class="rounded-lg border border-warning/40 bg-warning/10 p-4 text-sm"${_scopeId}><p class="mb-1 text-xs font-medium uppercase text-warning"${_scopeId}> Desglose del adeudo </p><p class="text-muted"${_scopeId}>${ssrInterpolate(fmtMoney(unref(normalPortion)))} de quincena normal + ${ssrInterpolate(fmtMoney(unref(carryoverPortion)))} de saldo arrastrado `);
							if (Number(__props.relation.total_late_fees ?? 0) > 0) _push(`<span${_scopeId}>(incluye ${ssrInterpolate(fmtMoney(__props.relation.total_late_fees))} de recargo por atraso)</span>`);
							else _push(`<!---->`);
							_push(` = <span class="font-semibold text-highlighted"${_scopeId}>${ssrInterpolate(fmtMoney(__props.relation.total_amount_due))}</span> total a pagar. </p></div>`);
						} else _push(`<!---->`);
						_push(`<div${_scopeId}><p class="mb-2 text-sm font-semibold text-highlighted"${_scopeId}> Vales incluidos </p>`);
						if (!__props.relation.items?.length) _push(`<div class="py-6 text-center text-sm text-muted"${_scopeId}> Este corte no incluye vales para esta relación (solo saldo arrastrado). </div>`);
							...
							ssrRenderList(unref(sortedItems), (item) => {
								...
								_push(`</td><td class="py-2 pr-3"${_scopeId}>${ssrInterpolate(fmtMoney(item.payment_amount))}</td><td class="py-2 pr-3"${_scopeId}>${ssrInterpolate(fmtMoney(item.commission_amount))} `);
								if (item.is_late_payment && Number(item.commission_forfeited_amount ?? 0) > 0) _push(`<p class="text-xs text-error"${_scopeId}> Perdida por atraso: ${ssrInterpolate(fmtMoney(item.commission_forfeited_amount))}</p>`);
								else _push(`<!---->`);
								_push(`</td><td class="py-2 pr-3"${_scopeId}>${ssrInterpolate(fmtMoney(item.late_fee_amount))}</td><td class="py-2 pr-3 text-right font-semibold text-highlighted"${_scopeId}>${ssrInterpolate(fmtMoney(item.line_total_amount))}</td></tr>`);
							});
							_push(`<!--]--></tbody></table></div>`);
						}
						_push(`</div><div class="grid grid-cols-2 gap-4 rounded-lg border border-default bg-elevated/50 p-4 sm:grid-cols-4"${_scopeId}><div${_scopeId}><p class="text-xs uppercase text-muted"${_scopeId}> Total pagos </p><p class="font-semibold text-highlighted"${_scopeId}>${ssrInterpolate(fmtMoney(__props.relation.total_payment))}</p></div><div${_scopeId}><p class="text-xs uppercase text-muted"${_scopeId}> Total comisión </p><p class="font-semibold text-highlighted"${_scopeId}>${ssrInterpolate(fmtMoney(__props.relation.total_commission))}</p>`);
						if (unref(totalCommissionForfeited) > 0) _push(`<p class="text-xs text-error"${_scopeId}> Se perdieron ${ssrInterpolate(fmtMoney(unref(totalCommissionForfeited)))} por atraso </p>`);
						else _push(`<!---->`);
						_push(`</div><div${_scopeId}><p class="text-xs uppercase text-muted"${_scopeId}> Total recargos </p><p class="font-semibold text-highlighted"${_scopeId}>${ssrInterpolate(fmtMoney(__props.relation.total_late_fees))}</p></div><div${_scopeId}><p class="text-xs uppercase text-muted"${_scopeId}> Total a pagar </p><p class="text-lg font-bold text-highlighted"${_scopeId}>${ssrInterpolate(fmtMoney(__props.relation.total_amount_due))}</p></div></div></div>`);
					} else _push(`<!---->`);
						...
						__props.relation.retroactive_reconciliation ? (openBlock(), createBlock("div", {
							key: 0,
							class: "rounded-lg border border-success/40 bg-success/10 p-4 text-sm"
						}, [createVNode("p", { class: "mb-1 text-xs font-medium uppercase text-success" }, " Pagada por conciliación manual "), createVNode("p", { class: "text-muted" }, [
							createTextVNode(" El depósito real sí llegó a tiempo según el banco: un gerente lo verificó el " + toDisplayString(fmtDateTime(__props.relation.retroactive_reconciliation.verified_at)), 1),
							Number(__props.relation.retroactive_reconciliation.waived_late_fees_total ?? 0) > 0 ? (openBlock(), createBlock("span", { key: 0 }, " y se le quitó " + toDisplayString(fmtMoney(__props.relation.retroactive_reconciliation.waived_late_fees_total)) + " de recargo por atraso que se había aplicado por error", 1)) : createCommentVNode("", true),
							createTextVNode(". Esta relación se quedó como CERRADA porque su deuda ya se arrastró a un corte más nuevo, pero ya no tiene ningún adeudo pendiente por esta causa. ")
						])])) : createCommentVNode("", true),
						unref(hasCarryover) ? (openBlock(), createBlock("div", {
							key: 1,
							class: "rounded-lg border border-warning/40 bg-warning/10 p-4 text-sm"
						}, [createVNode("p", { class: "mb-1 text-xs font-medium uppercase text-warning" }, " Desglose del adeudo "), createVNode("p", { class: "text-muted" }, [
							createTextVNode(toDisplayString(fmtMoney(unref(normalPortion))) + " de quincena normal + " + toDisplayString(fmtMoney(unref(carryoverPortion))) + " de saldo arrastrado ", 1),
							Number(__props.relation.total_late_fees ?? 0) > 0 ? (openBlock(), createBlock("span", { key: 0 }, "(incluye " + toDisplayString(fmtMoney(__props.relation.total_late_fees)) + " de recargo por atraso)", 1)) : createCommentVNode("", true),
							createTextVNode(" = "),
							createVNode("span", { class: "font-semibold text-highlighted" }, toDisplayString(fmtMoney(__props.relation.total_amount_due)), 1),
							createTextVNode(" total a pagar. ")
						])])) : createCommentVNode("", true),
						createVNode("div", null, [createVNode("p", { class: "mb-2 text-sm font-semibold text-highlighted" }, " Vales incluidos "), !__props.relation.items?.length ? (openBlock(), createBlock("div", {
						...
						])]), createVNode("tbody", null, [(openBlock(true), createBlock(Fragment, null, renderList(unref(sortedItems), (item) => {
							return openBlock(), createBlock("tr", {
								...
								createVNode("td", { class: "py-2 pr-3" }, [createTextVNode(toDisplayString(fmtMoney(item.commission_amount)) + " ", 1), item.is_late_payment && Number(item.commission_forfeited_amount ?? 0) > 0 ? (openBlock(), createBlock("p", {
									key: 0,
									class: "text-xs text-error"
								}, " Perdida por atraso: " + toDisplayString(fmtMoney(item.commission_forfeited_amount)), 1)) : createCommentVNode("", true)]),
						...
						createVNode("div", { class: "grid grid-cols-2 gap-4 rounded-lg border border-default bg-elevated/50 p-4 sm:grid-cols-4" }, [
							...
							createVNode("div", null, [
								createVNode("p", { class: "text-xs uppercase text-muted" }, " Total comisión "),
								createVNode("p", { class: "font-semibold text-highlighted" }, toDisplayString(fmtMoney(__props.relation.total_commission)), 1),
								unref(totalCommissionForfeited) > 0 ? (openBlock(), createBlock("p", {
									key: 0,
									class: "text-xs text-error"
								}, " Se perdieron " + toDisplayString(fmtMoney(unref(totalCommissionForfeited))) + " por atraso ", 1)) : createCommentVNode("", true)
							]),
...
var cutoffs_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	async setup(__props) {
		...
		[__temp, __restore] = withAsyncContext(() => fetchMe()), await __temp, __restore();
					...
					if (_push) if (unref(status) === "pending") {
						...
						if (unref(items).length === 0) {
							...
							ssrRenderList(unref(items), (cutoff) => {
								...
								_push(`</div><div class="min-w-0"${_scopeId}><p class="truncate font-semibold text-highlighted"${_scopeId}> Corte #${ssrInterpolate(cutoff.id)} · ${ssrInterpolate(branchName(cutoff.branch_id))} · ${ssrInterpolate(fmtPeriodStart(cutoff.period_start))} – ${ssrInterpolate(fmtPeriodEnd(cutoff.scheduled_at))}</p><p class="text-xs text-muted"${_scopeId}>${ssrInterpolate(cutoff.cutoff_type ?? "PAGOS")} · Ejecutado ${ssrInterpolate(fmtDate(cutoff.executed_at))} `);
...
//# sourceMappingURL=cutoffs-BLwH-l4I.mjs.map
