...
<template>
  <div v-if="items.length === 0" class="flex flex-col items-center justify-center py-16 text-center">
    ...
    <p class="mt-2 text-sm text-muted">
      No hay incrementos de crédito pendientes
    </p>
  </div>
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex min-w-0 items-center gap-3">
          ...
          <div class="min-w-0">
            ...
            <p v-if="item.reason" class="text-xs text-dimmed line-clamp-1">
              {{ item.reason }}
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          <div class="text-right">
            <p class="text-xs text-muted">
              Solicitado:
              <span class="font-medium text-highlighted">{{ currency.format(Number(item.requested_amount)) }}</span>
            </p>
            <p class="text-sm font-semibold text-highlighted">
              Pre-autorizado: {{ currency.format(Number(item.pre_authorized_amount || 0)) }}
            </p>
          </div>
